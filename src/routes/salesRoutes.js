const express = require('express');
const router = express.Router();
const prismaManager = require('../utils/create/prismaManager');  
const { logger } = require('../config/logger');

// Получение списка продаж
router.get('/', async (req, res) => {
  try {
    const sales = await prismaManager.paginate('sales', {
      page: parseInt(req.query.page) || 1,
      perPage: parseInt(req.query.perPage) || 10,
      where: {
        user_id: parseInt(req.query.userId),
        client_id: req.query.clientId ? parseInt(req.query.clientId) : undefined,
        warehouse_id: req.query.warehouseId ? parseInt(req.query.warehouseId) : undefined,
        status: req.query.status,
        doc_date: {
          gte: req.query.startDate ? new Date(req.query.startDate) : undefined,
          lte: req.query.endDate ? new Date(req.query.endDate) : undefined
        }
      },
      orderBy: {
        doc_date: 'desc'
      },
      include: {
        client: true,
        warehouse: true,
        users: {
          select: {
            username: true
          }
        }
      }
    });
    res.json(sales);
  } catch (error) {
    logger.error('Error getting sales:', error);
    res.status(500).json({ error: 'Failed to get sales' });
  }
});

// Создание продажи
router.post('/', async (req, res) => {
  try {
    const sale = await prismaManager.prisma.sales.create({
      data: {
        doc_number: req.body.doc_number,
        doc_date: new Date(req.body.doc_date),
        sale_date: req.body.sale_date ? new Date(req.body.sale_date) : null,
        user_id: parseInt(req.body.user_id),
        client_id: parseInt(req.body.client_id),
        warehouse_id: parseInt(req.body.warehouse_id),
        total_amount: parseFloat(req.body.total_amount),
        currency: req.body.currency,
        status: req.body.status || 'draft',
        invoice_type: req.body.invoice_type,
        invoice_number: req.body.invoice_number,
        vat_rate: req.body.vat_rate ? parseFloat(req.body.vat_rate) : null
      },
      include: {
        client: true,
        warehouse: true,
        users: {
          select: {
            username: true
          }
        }
      }
    });
    res.status(201).json(sale);
  } catch (error) {
    logger.error('Error creating sale:', error);
    res.status(500).json({ error: 'Failed to create sale' });
  }
});

// Получение продажи по ID
router.get('/:id', async (req, res) => {
  try {
    const sale = await prismaManager.prisma.sales.findUnique({
      where: { id: parseInt(req.params.id) },
      include: {
        client: true,
        warehouse: true,
        users: {
          select: {
            username: true,
            email: true
          }
        }
      }
    });
    if (!sale) {
      return res.status(404).json({ error: 'Sale not found' });
    }
    res.json(sale);
  } catch (error) {
    logger.error('Error getting sale:', error);
    res.status(500).json({ error: 'Failed to get sale' });
  }
});

// Обновление продажи
router.put('/:id', async (req, res) => {
  try {
    const sale = await prismaManager.prisma.sales.update({
      where: { id: parseInt(req.params.id) },
      data: {
        doc_number: req.body.doc_number,
        doc_date: new Date(req.body.doc_date),
        sale_date: req.body.sale_date ? new Date(req.body.sale_date) : null,
        client_id: parseInt(req.body.client_id),
        warehouse_id: parseInt(req.body.warehouse_id),
        total_amount: parseFloat(req.body.total_amount),
        currency: req.body.currency,
        status: req.body.status,
        invoice_type: req.body.invoice_type,
        invoice_number: req.body.invoice_number,
        vat_rate: req.body.vat_rate ? parseFloat(req.body.vat_rate) : null
      }
    });
    res.json(sale);
  } catch (error) {
    logger.error('Error updating sale:', error);
    res.status(500).json({ error: 'Failed to update sale' });
  }
});

// Удаление продажи
router.delete('/:id', async (req, res) => {
  try {
    await prismaManager.prisma.sales.delete({
      where: { id: parseInt(req.params.id) }
    });
    res.status(204).send();
  } catch (error) {
    logger.error('Error deleting sale:', error);
    res.status(500).json({ error: 'Failed to delete sale' });
  }
});

// Изменение статуса продажи
router.patch('/:id/status', async (req, res) => {
  try {
    const sale = await prismaManager.prisma.sales.update({
      where: { id: parseInt(req.params.id) },
      data: {
        status: req.body.status
      }
    });
    res.json(sale);
  } catch (error) {
    logger.error('Error updating sale status:', error);
    res.status(500).json({ error: 'Failed to update sale status' });
  }
});

module.exports = router; 