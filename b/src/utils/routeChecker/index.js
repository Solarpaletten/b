...
const backendUrl =
  process.env.FRONTEND_URL?.replace('3000', '4000') || 'http://localhost:4000';
...
async function проверитьСтруктуру() {
  ...
  for (const [pageFile, route] of Object.entries(
    expectedStructure.frontend.pages
  )) {
  ...
  for (const controller of expectedStructure.backend.controllers) {
    const exists = fs.existsSync(
      path.join(backendDir, 'controllers', controller)
    );
